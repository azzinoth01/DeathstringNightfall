...
void UMIAnimInstance::ComputeSurfaceImpacts()
{
	...
	for (FMISurfaceImpact& Impact : SurfaceImpacts)
	{
		...
		{
			UE_LOG(LogTemp, Warning, TEXT("Socket { %s } does not exist on { %s }, can not play impact/footstep"), *Impact.Socket.ToString(), *GetNameSafe(Mesh->GetSkinnedAsset()));
			continue;
		}

		if (!Mesh->DoesSocketExist(Impact.Bone.BoneName))
		{
			UE_LOG(LogTemp, Warning, TEXT("Bone { %s } does not exist on { %s }, can not play impact/footstep"), *Impact.Socket.ToString(), *GetNameSafe(Mesh->GetSkinnedAsset()));
			continue;
		}
