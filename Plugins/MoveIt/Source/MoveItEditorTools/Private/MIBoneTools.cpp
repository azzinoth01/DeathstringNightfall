...
void UMIBoneTools::ErrorCantFindBone(USkeleton* Skeleton, const FName BoneName) const
{
...
void UMIBoneTools::SaveAndReloadAsset(const UObject* Asset) const
{
...
bool UMIBoneTools::FindBone(const TArray<FMeshBoneInfo>& BoneInfos, const TArray<FTransform>& BoneTMs, USkeleton* Skel, const FName& BoneName, FMeshBoneInfo& OutBoneInfo, FTransform& OutBoneTM, int32& OutBoneIndex) const
{
...
void UMIBoneTools::AddBoneToSkeleton(FMIBoneInfo& BoneInfo, TArray<USkeletalMesh*> Meshes)
{
	...
	{
		if (SkelMesh->GetRefSkeleton().FindRawBoneIndex(BoneInfo.BoneInfo.Name) == INDEX_NONE)
		{
			FReferenceSkeletonModifier Mod = FReferenceSkeletonModifier(SkelMesh->GetRefSkeleton(), SkelMesh->GetSkeleton());
			Mod.Add(FMeshBoneInfo(BoneInfo.BoneInfo.Name, BoneInfo.BoneInfo.ExportName, BoneInfo.BoneInfo.ParentIndex), BoneInfo.TM);

			BoneInfo.Index = SkelMesh->GetRefSkeleton().FindRawBoneIndex(BoneInfo.BoneInfo.Name);
		}
	}
}
...
)
{
	...
	USkeleton* const Skeleton = (bSkeletonBased) ? FromSkeleton : FromMesh->GetSkeleton();
