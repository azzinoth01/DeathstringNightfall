...
public:
	...
	void ErrorCantFindBone(USkeleton* Skeleton, const FName BoneName) const;
	...
	static void UpdateBoneNameForIK(FMeshBoneInfo& BoneInfo, const FString& IKPrefix, const FString& IKSuffix);

	bool FindBone(const TArray<FMeshBoneInfo>& BoneInfos, const TArray<FTransform>& BoneTMs, USkeleton* Skel, const FName& BoneName, FMeshBoneInfo& OutBoneInfo, FTransform& OutBoneTM, int32& OutBoneIndex) const;

	static void AddBoneToMesh(USkeletalMesh* const Mesh, int32 LODIndex, int32 BoneIndex);

	static void AddBoneToSkeleton(FMIBoneInfo& BoneInfo, TArray<USkeletalMesh*> Meshes);
